...
const menuItems = [
  ...
  { id: 'staff', label: '학관매 관리', icon: Users },
...
export function AdminSidebar({ activeMenu, onMenuChange }: SidebarProps) {
  return (
    <aside className="flex h-full w-[240px] shrink-0 flex-col border-r border-border bg-secondary/50">
      {/* Logo */}
      <div className="flex h-[60px] items-center px-6">
        <span className="text-lg font-bold text-primary">{'APM'}</span>
      </div>
      ...
      <div className="border-t border-border px-4 py-4">
        ...
        <p className="text-xs text-muted-foreground">{'운기매'}</p>
      </div>
    </aside>
  )
}
